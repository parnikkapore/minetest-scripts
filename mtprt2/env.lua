...
F.stn = {
    ...
    Amy = {name = "Armya",      stops = {"Amy"}},
    Ckw = {name = "Clarkwich",  stops = {"CkwN", "CkwS"}},
    Cld = {name = "Cold",       stops = {"CldN", "CldS"}},
    IrS = {name = "Ironshore",  stops = {"IrSW", "IrSE"}},
    WdM = {name = "Wildmouth",  stops = {"WdMN", "WdMS"}}
}

function F.waypoint(name)
    __approach_callback_mode = 1
    wp_last_handled_train = wp_last_handled_train or -1

    if event.approach and event.id ~= wp_last_handled_train and (not event.has_entered) then
        wp_last_handled_train = event.id

        if get_line() ~= "-" then
            if F.tkfirst(get_rc()) ~= name then
                print(name .. ": Current leg mismatch. Got " .. F.tkfirst(get_rc()))
                local new_route = F.route(name, get_line())
                if not new_route then
                    print(name .. ": No route from " .. name .. " to " .. get_line() .. " - cannot recover! Check manually!")
                    return
                end
                set_rc(new_route)
            end
    ...
    elseif event.train then
        wp_last_handled_train = -1 -- This is really to dedupe approach events
...
function F.stnguard(px,py,pz,sname)
  ...
  if event.approach and event.id ~= last_handled_train and (not event.has_entered) then
      last_handled_train = event.id
      local will_dep = (get_line()=="-" or get_line()==sname) and reserveCount > 0 and can_set_route(p, "enter")
...
function F.route(from, to)
    ...
    if (not F.stn[to]) or (not S.dists[from]) then return nil end
    ...
    if best_plat == nil then return nil
    else return r(from, best_plat)
    end
end
...
function F.junc(px, py, pz, paths)
    ...
    if event.approach and event.id ~= last_handled_train and (not event.has_entered) then
        ...
        if get_line() ~= "-" then
            ...
            -- Current wpt hasn't been popped yet. cadr time!
...
function F.tkrest(tokens) return tokens:match("%S+ (.*)") or "" end
